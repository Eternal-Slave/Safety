...
export const run: StrSelectRun = async (client, interaction) => {
    ...
    await interaction.editParent({ content: `${emojis.loading} Updating auto-ban subscriptions...`, components: [] });

    const autoBans = interaction.data.values.getStrings();
    guild = await updateGuild(interaction.guildID!, { $set: { 'safety.autoBan': autoBans } });
    await interaction.editOriginal({ content: `${emojis.success} Updated auto-ban subscriptions.` });
...
export const info: IntInfo = {
    ...
    id: 'select.gctl.safety.auto-ban'
};
